...
"""examples of kernels transofrmed in mod2 from hex value
...
""" Translate hex values (weights) into mod2 values using math formulas given by Maxim
    formula: w = (-a_7 2^7+a_6 2^6+a_5 2^5+a_4 2^4+a_3 2^3+a_2 2^2+a_1 2^1+a_0)/128
Returns:
    int: number in mod2
"""
...
def mod2(val):
  ...
  Args:
      val (int): number to transform

  Returns:
      int8_t: return a 8bit value that correspond to the hex number in mod2
  """  
...
def transform_mod2(list,name):
  """transform a 32bit hex values array an array of mod2 numbers
     I used it to print the values 

  Args:
      list (int): list of hex values to transform (kernels)
      name (string): name to give to the array
  """  
  print(str(name),end='=[')
  for h in list: 
